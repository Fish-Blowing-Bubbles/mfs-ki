    ...
    while(document.getElementById("q" + i)) {
        ...
        if(savedAnswer) {
            ...
            if(document.getElementById(`q${i}_a`)) {
                ...
                // answer type mixed (multiple choice and freeform)
                if (document.getElementById(`q${i}_a0`)) {
                    // if a multiple choice answer exists, highlight it
                    if (document.getElementById(`q${i}_${savedAnswer.split(':::')[0]}`)) {
                        document.getElementById(`q${i}_${savedAnswer.split(':::')[0]}`).classList.add("question-answer-highlighted");
                    }
                    // if a text answer exists, write it into the textarea
                    if (savedAnswer.includes(':::')) {
                        document.getElementById(`q${i}_a`).value = savedAnswer.split(':::')[1];
                    }
                } else {
                ...
                // multiple-choice answer with multiple correct solutions
                const selectedAnswers = savedAnswer.split(',');
                selectedAnswers.forEach(answerID => {
                    document.getElementById(`q${i}_${answerID}`).classList.add("question-answer-highlighted");
                });
            } else {
 ...
 * @param {String} type the answer type - default, mixed (multiple-choice and text answer) or multi (multiple options for multiple-choice are possible)
 */
function selectAnswer(element, type='default') {
    ...
    // remove the highlight from the old answer - not required for multi answers
    if(questionAnswers[questionID] != undefined && type != 'multi') {
        let oldAnswerID = questionAnswers[questionID];

        // find the saved answer in the mixed case
        if (type == 'mixed') {
        ...
        if (oldAnswerElement != null) {
            if (type == 'default' || type == 'mixed') {
    ...
    if (questionAnswers[questionID] != undefined && type == 'mixed') {
        questionAnswers[questionID] = questionAnswers[questionID].replace(questionAnswers[questionID].split(':::')[0], answerID);
    } 
    // save all selected answers in a comma separated list in case of a multi question
    else if (type == 'multi') {
        let answersString = '';
        selectedAnswers.forEach(id => answersString += id + ',');
        questionAnswers[questionID] = answersString.slice(0, -1);
    } 
    // save the plain answerID in the default case
    else {
...
function updateTextBox(element, type='default') {
    ...
    if (type == 'default') {
        ...
        // case A - no answer exists yet
        if (!questionAnswers[questionID]) {
            questionAnswers[questionID] = ':::' + element.value.replace("<script>", "").replace("</script>", "");
        }
        // case B - only a multiple choice answer exists so far
...
function loadScreen(screenID, storeScreen = true) {
    ...
    if (storeScreen) {
        ...
        sessionStorage.setItem("active_question", activeQuestionID); // set the session storage right away to make the behavior more predictable
    }
...
function isMixedQuestion(questionElement) {
...
function restartQuestionnaire(clearSession=true) {
    ...
    if (clearSession) loadScreen("start");
}
